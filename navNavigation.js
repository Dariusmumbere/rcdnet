...
async function loadDirectorDashboard() {
    try {
        ...
        const approvals = await getPendingBudgetApprovals();
        const approvalsContainer = document.getElementById('pendingApprovalsContainer');
        
        if (approvals.length === 0) {
            approvalsContainer.innerHTML = '<p>No pending budget approvals</p>';
            return;
        }
        
        approvalsContainer.innerHTML = `
            <h3><i class="fas fa-clipboard-check"></i> Pending Budget Approvals</h3>
                    ...
                    <tbody id="pendingApprovalsTableBody">
                        ${approvals.map(approval => `
                            <tr>
                                ...
                                <td>${approval.budget.toLocaleString()}</td>
                                <td>${new Date(approval.created_at).toLocaleDateString()}</td>
                                <td>
                                    <button class="action-btn approve-btn" onclick="approveBudget(${approval.id})">
                                        <i class="fas fa-check"></i> Approve
                                    </button>
                                    <button class="action-btn reject-btn" onclick="rejectBudget(${approval.id})">
...
// Add budget approval functions
async function approveBudget(approvalId) {
    const remarks = prompt('Enter approval remarks (optional):');
    try {
        await processBudgetApproval(approvalId, true, remarks);
...
async function rejectBudget(approvalId) {
    ...
    try {
        await processBudgetApproval(approvalId, false, remarks);
        loadDirectorDashboard();
    } catch (error) {
